# TODO from https://github.com/NixOS/nixpkgs/pull/309327, temporarily copied here until packaged in nixpkgs
{
  lib,
  fetchFromGitHub,
  fetchzip,
  ...
  wrapper-to-env = wrapper:
    if lib.isDerivation wrapper
    then lib.getExe wrapper
  ...
  buildDotnetModule {
    ...
    preInstall = ''
      ...
      makeWrapper ${phome}/find-love $out/bin/olympus \
        ...
        --set-default OLYMPUS_SKIP_SCHEME_HANDLER_CHECK ${
        if skipHandlerCheck
        then "1"
        else "0"
      } \
